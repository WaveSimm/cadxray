...
# FreeCADDiag/gui.py가 한다. 이 파일이나 gui.py를 고치면 FreeCAD를 재시작한다.

import os
import sys

for _entry in list(sys.path):
    if os.path.basename(os.path.normpath(_entry)) == "FreeCADDiag":
        _parent = os.path.dirname(os.path.normpath(_entry))
        if _parent and _parent not in sys.path:
            sys.path.insert(0, _parent)
        break

from FreeCADDiag import gui as _diag_gui
