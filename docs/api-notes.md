...
- **절점 공유는 맞닿은 두 면이 같은 면일 때만** `[라이브 1.1.3, 2026-09-18]`: 원판(R20) 위에 작은 원통(R10)을 얹은 Compound는 접합면 공유 절점 0개 → 위 부품이 따로 놀아 변위 5e11. `a.generalFuse([b], 1e-6)`로 조각내면 접합면이 서로 새겨져(솔리드 수·부피 그대로) 공유 절점 291개. 겹치는 부품은 조각이 늘어나 부피가 안 맞는다(그걸로 interference 검출). Gmsh 그룹 `SolidN_Nodes`(Node)의 교집합으로 공유 여부를 셀 수 있다
- **`Part.Compound`에는 `CenterOfMass`가 없다** `[라이브 1.1.3]`: `makeCylinder().cut()` 결과·MultiFuse·Body 일부는 Compound. `shape.Solids[i].CenterOfMass`로
- **`SecondOrderLinear=True`(중간 절점을 직선 변에)는 곡면에서 FreeCAD의 기하 조회를 깨뜨린다** `[라이브 1.1.3, 2026-09-18]`: 중간 절점이 곡면에서 h²/(8r)만큼 벗어나 `getNodesBySolid`가 곡면 위 절점(r=20.000)을 밖으로 분류 → 5111개 중 949개 요소가 어느 재질 ELSET에도 안 들어가 CalculiX `*ERROR in calinput: no material was assigned to element`. 같은 이유로 원통면에 `getccxVolumesByFace`·`getFacesByFace`가 0개(`getNodesByFace`는 411개) → 힘·압력의 `*CLOAD`/`*DLOAD`가 비고 해석은 정상 종료해 응력 0. `SecondOrderLinear=False`(휜 요소)나 1차 요소면 전부 정상(752개). 평면만 쓰는 외팔보·상자는 영향 없음
- **재질 ELSET을 Gmsh 그룹으로 정확히**: Gmsh 메시에는 `Solid1`·`Solid2`(Volume, 요소를 정확히 한 번씩 분할)·`FaceN`·`SolidN_Nodes` 그룹이 기본으로 들어온다(`GroupCount` 36). 작성기(`meshsetsgetter.get_solid_element_sets`)는 `GroupCount>0`이면 재질 객체 **이름 + "_"로 시작하는 Volume 그룹**을 그대로 쓴다(`meshtools.get_femmesh_groupdata_sets_by_name`). `fm = mesh.FemMesh; gid = fm.addGroup("Material_A_elements", "Volume"); fm.addGroupElements(gid, ids); mesh.FemMesh = fm`로 복사하면 된다(`removeGroup`·`renameGroup`도 있음). 단일 재질(References 없음)은 그룹을 못 찾아 기존 경로(나머지 요소 전체)로 간다
- **.inp 형식**: 단일 재질은 `*ELSET,ELSET=MaterialSolid` 아래 데이터 줄이 요소 번호가 아니라 집합 이름 `Evolumes`. 2차 요소는 `id, n1..n9,` 뒤 줄바꿈 후 `n10`(끝의 콤마가 이어짐 표시). `*SOLID SECTION, ELSET=…, MATERIAL=…`. 힘은 `*CLOAD` 뒤 `node,dof,value` 줄, 압력·자중은 `*DLOAD`. `fea.inp_file_name`이 경로
- 원통 R10 h40 옆면에 100 N(+X), 바닥 고정, 휜 2차 3 mm: 최대 von Mises 2.98 MPa(굽힘 이론 2.55 + 뿌리 국소), CLOAD 1546줄, 1.3 s — 휜 2차 요소도 단순 곡면에서는 jacobian 문제 없음
